...
# ==========================================================
# Script de Experimentos TFG: AQM & L4S (v7.0-P2 - RPi Testbed)
# Objetivo: Comparativa pfifo, fq_codel y L4S (DualPI2)
#           con 2 FLUJOS PARALELOS HOMOGÉNEOS (iperf -P 2)
...
DURACION=60
VENTANA="2000K"
NUM_FLUJOS=2                    # <-- Número de flujos paralelos homogéneos (-P)
RESULTADOS=~/resultadosFinales_paraleloCA
...
# ============================================
# Función: Ejecutar Escenario (con -P flujos paralelos homogéneos)
# ============================================
ejecutar_escenario() {
    local escenario=$1
    local bw_limit=$2
    ...
    echo ""
    echo "=========================================================="
    echo " EJECUTANDO: $escenario (CCA=$cca | ECN=$ecn_val | BW=$bw_limit | P=$NUM_FLUJOS)"
    echo "=========================================================="
    ...
    # NOTA: con -P, todos los flujos comparten el mismo CCA/ECN (homogéneos)
    ...
    # Captura tcpdump en segundo plano (incluye ambos flujos, mismo puerto 5001)
    ...
    # Arrancar servidor iperf en rp52 (UN SOLO servidor: -P no necesita puertos extra)
    ...
    # Captura ss en cliente (rp51) cada 0.5s en segundo plano (verá ambos flujos, mismo destino)
    ...
    # Ejecutar iperf cliente en rp51 CON -P (flujos paralelos homogéneos)
    echo "[*] Lanzando iperf cliente en rp51 con -P $NUM_FLUJOS flujos paralelos..."
    ssh ${IPERF_SERVER_USER}@${CLIENTE} \
        "${IPERF_PATH} -c $SERVIDOR -P $NUM_FLUJOS -e -i 1 -t $DURACION -w $VENTANA $IPERF_EXTRA" \
    ...
    echo "    - ${escenario}_cliente.txt   (contiene [1], [2] y [SUM-2])"
...
# ============================================
echo "=========================================================="
echo " TFG - Testbed AQM con Raspberry Pi (Flujos Paralelos -P)"
echo " rp51 (cliente: $CLIENTE) <-> rp50 (router) <-> rp52 (servidor: $SERVIDOR)"
echo " Número de flujos paralelos homogéneos: $NUM_FLUJOS"
echo "=========================================================="
echo ""
...
echo ""
echo "[*] Iniciando experimentos con $NUM_FLUJOS flujos paralelos homogéneos..."
...
echo ""
echo "=========================================================="
echo " EXPERIMENTOS COMPLETADOS (FLUJOS PARALELOS -P $NUM_FLUJOS)"
