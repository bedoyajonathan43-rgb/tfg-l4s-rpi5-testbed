#!/usr/bin/env python3
"""
Extrae throughput medio y RTT medio de los ficheros _cliente.txt generados
por iperf2 (-e) para cada escenario (E1-E4) y cada lote de ancho de banda,
en la campaña de flujo individual.

Uso:
    Ejecutar desde la raíz del repositorio, con los resultados ya
    descargados en ./resultados/resultadosFinales_1_flujoC/<lote_bw>/...

    python3 scripts/parsing/extraer_metricas.py

Genera: ./metricas/metricas_individual.csv
"""
import re
import csv
...
BASE = REPO_ROOT / "resultados" / "resultadosFinales_1_flujoC"
OUT_CSV = REPO_ROOT / "metricas" / "metricas_individual.csv"
...
# Orden lógico de los lotes de BW (baseline aparte, sin límite real)
...
# Línea de intervalo: [  1]  0.00-1.00 sec  ...  Bandwidth ...  RTT(var)) us
INTERVAL_RE = re.compile(
    r"\[\s*\d+\]\s+([\d.]+)-([\d.]+)\s+sec\s+.*?\s+([\d.]+)\s+(Mbits|Kbits|Gbits)/sec.*?"
    r"(\d+)\((\d+)\)\s+us"
)

# Línea resumen final: [  1]  0.00-XX.XX sec  Transfer  Bandwidth  Write/Err  Rtry ...
...
def parse_client_file(path: Path):
    ...
    # --- Throughput medio y retransmisiones: línea resumen (0.00-total) ---
    avg_thr_mbps = None
    total_dur = None
    writes = retries = None
    for line in lines:
    ...
    # --- RTT medio: intervalos de 1s, descartando warm-up (t<2s) y el
    #     intervalo final parcial (que suele ser < 1s) ---
    rtts_us = []
    for line in lines:
        ...
        rtt_us = int(m.group(5))
        if t0 < 2.0:
            continue  # descartar arranque / slow start
        if (t1 - t0) < 0.99:
            continue  # descartar intervalo final parcial
...
def main():
    if not BASE.exists():
        raise SystemExit(
            ...
            f"Copia ahí los resultados de la campaña de flujo individual "
            f"(estructura: resultados/resultadosFinales_1_flujoC/<lote_bw>/...)."
        )
